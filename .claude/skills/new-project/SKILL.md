...
2. **Living doc** — create `ctrl/docs/projects/<name>.md`: one-line what/why, status line (dated), design principle, plan, open questions. Skip if it exists (it usually will — ideas start in ctrl before the repo). A doc that outgrows one file may become `ctrl/docs/projects/<name>/` with a `README.md` entry point (see workflow.md).
