...
3. **Follow the setup doc** — README's "Local setup" section: install prereqs/tooling, project deps, any codegen. Check the ctrl living doc (`docs/projects/<name>.md`, or `docs/projects/<name>/`) for personal/machine-specific steps the README can't hold (secrets, signing, service credentials — typically in Vaultwarden).
4. **Verify** — run the documented `check`, build, and test commands; setup isn't done until they pass.
