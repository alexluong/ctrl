# Team log
- 2026-09-26 — **Code review wave 2 complete** (dev): B5 B6 B7 B9 B10 B12 B13 B14 B15-partial A3, plus N52's slug and QA's N66. **solex `f77cad4`, staging `70dd7c8c`, 560 unit green**, biome baseline, migration 0021 (`approvals.method`) applied dev + remote. Per landing: B12+B14 `aa0934c`, B5+B6+N66 `8de0f3f`, B7 `9de8f1b`, B9+B13 `8dfd98e`, B15 `98c9de0`, B10+A3+N52 `f77cad4`. **The review's fixes uncovered two faults of ours that it had not seen:** booking and stay ids were minted *outside* `plan()` against D-12 (f) — latent until B7 gave two group creates something to collide on, at which point a retry reused the losing attempt's stream ids and the command could never succeed; and `approvalDecide.request`'s doc comment claimed the underlying command's rules ran at request time, which they did not. Both now true. **Two deliberate deviations, in the commits:** B13 asks "is there an active owner" rather than "is this the first row" (the second state is unreachable — the last-owner guard refuses first — so both guards are pinned in one test); B15's roll-hour default change moves nothing already written, because every event stores the business date it was given, and the comment claiming otherwise was corrected. **Deferred, product-spec'd, architect schedules:** the `{key, params}` stored-memo schema + `HotelProfile.locale`. Dev idle.
- 2026-09-26 — Dev, wave 2 in progress: **B12+B14 `aa0934c`** (QA verified S6-6/S6-7), **B5+B6+N66 `8de0f3f`**, staging `ab5e7ca6`, 550 unit green. **For QA — one on-screen wording change, unavoidable:** B14 made `folio.refundable` and `error.folio.refundTooLarge` untrue ("that is what has been taken in" is no longer the number once part of a deposit is kept), so both now say "less anything kept". Architect asked wave 2 to avoid wording changes; this one described a number the code no longer computes. **If a filmed journey shows the refund line it needs a re-film.** B6's dangerous branch (an owner resetting the system operator's password → sign in → `/system` raw log) is not reachable from the screens, because the operator flag cannot be set by any command — the rule was moved into the pure domain so both branches are tested rather than left to a guard nobody could exercise. Still to come: B7, B9, B10, B13, B15, A3, N52's slug.
- 2026-09-26 — Player republished (v3) on QA's re-film: nine journeys on solex d10cae5 (staging 8cbd00c0), 11/11, Oanh/Dao; R9 now shows the N64 notice and the N65 fix (nobody listed after both erases; N65 found by collie-lab off a local run). N61–N65 all fixed. Architect scheduling for N51/N52 (product-ruled 661d15c): N52 company-name-at-render fix goes with wave 2 now (small, on the bill); the `{key, params}` memo schema is a follow-up (schema change, not a demo gate) → mvp.md §4. Wave 2 re-sent to dev (S6-6/7 wait on B12, B14). Film only 00:00–23:00 hotel time (QA). collie-lab (Alex's replay side project) was given a read-only local recipe: own worktree `solex-lab`, fresh-db script with SOLEX_DEV_USER, N64 state per R9.
- 2026-09-25 — Review package complete (architect catch-up): dev landed wave 1 (B1 6de629c, B2 a758801, B3 a7f975f, B4 bcaceeb, B8 a9baa51) + N46–N64 fixes (ebdaaab, c4b0350, 9f38914); QA scripted R6–R9 (5790c7a, de56247, 17b938d), suite green on ceece38, journeys seed + R1–R9 + English sweep 11/11 on c4b0350 (staging 4289576d), all nine filmed 21:24–21:34 with Oanh/Dao; product's "In ezFolio today" in all nine docs. Player republished with nine journeys.
- 2026-09-25 — QA verified the N64 interim from the screens (S4-31, S4-32; e2e 6cbbe8c/388e528), including the case dev was least sure of: after erasing the offered counterpart, **no** notice points back at the record just erased — the stale-loader trap the fix exists to avoid. S4-32 confirms silence when there is no counterpart. R9 (not R8) films the second erase. Re-filming all nine on **solex `9f38914`, staging `924aa3c3`, 540 unit green**.
- 2026-09-25 — QA S6-8 proves the wave-1 collision fix **from the screens**: a guest who slept one night and leaves a day early on account is asked for one night, the company is invoiced one night, and the guest's bill is zero. That is the B1×B3 interaction that killed transfer-to-receivable, checked from outside rather than only by the domain test dev wrote alongside the fix. Suite on `ceece38` was desk 155/159 + receptionist 13/14; all five reds were QA's own specs (four expecting the old refusal wording, one expecting a reversal where the roll had not posted the night — the app is right: the reversal is conditional on a live charge, freeing without reversing is the correct other path). Filming all nine on `c4b0350`.
- 2026-09-25 — **For architect, who asked after its session restarted and could not be replied to** (three questions, all already answered upstream): (1) wave-1 sha is **solex `ceece38`, staging `e47f2ef2`, 540 unit green** — B4 `bcaceeb` and B8 `a9baa51` are in. (2) B1 same-day: already on minimum-one-night; product amended §10 6a themselves (`79631fd`) and QA caught dev mid-flight running the earlier "pays zero" version. (3) N51/N52 are **ruled by product** (`661d15c`): typed text stored as typed, system-written lines become `{key, params}` rendered at read time, printed bill uses a new `HotelProfile.locale`, never store a slug where a name shows. Product's closing line was "architect decides the wave", so **the only open question is scheduling, not design** — dev has not started it because it is a schema change, not a filming fix. Two things left for architect: **wave 2 is unblocked** (B5, B6, B7, B9, B10, B12, B13, B14, B15, A3), and **N52's slug is separable and tiny** — the company-name-at-render fix is a couple of lines, independent of the rest, and removes the only thing on a bill today that is wrong in any language.
- 2026-09-25 — Dev: **code review wave 1 landed** — solex `ceece38`, staging `e47f2ef2`, 540 unit green, biome at baseline. B1 `6de629c`, B2 `a758801`, B3 `a7f975f`, B4 `bcaceeb`, B8 `a9baa51`, follow-through `ceece38`; QA N46–N62 and N57 in. **Two correct rulings made a third thing impossible:** B1 (reverse today's night) and B3 (refuse a check-out in credit) each right, together they killed transfer-to-receivable — the desk settles *before* check-out, so the transfer sent the whole balance, the company was invoiced for a night nobody slept, and the folio went into credit where B3 refused the check-out. R3's check-out beat, dead, with no single ruling wrong; found only because QA filmed a journey crossing both. Fixed with one server-side answer to "what must they settle to leave today", used by the settle dialog and the transfer. Deliberate deviations: B4 has no posted-night guard (the roll posts tonight before anybody moves anybody, so it would make an in-house move impossible); B1's same-day case went round twice and **QA caught dev mid-flight with the wrong version** before it shipped. `codes.test.ts` had been green over a missing sentence because a semicolon inside a doc comment ended the union early — the guard blind to its own bug class; fixed, nothing else hiding. Open with product, not blocking: the literal reading of §10 6a for a multi-night stay leaving on arrival day, and whose language a *stored* memo is in (N51/N52 — the raw company slug is a bug either way). **Wave 2 unblocked.**
