## 2026-09-25 — code review wave 1 landed (dev)

**solex `ceece38`, staging `e47f2ef2`, 540 unit green, biome at baseline.** B1 `6de629c`, B2 `a758801`, B3 `a7f975f`, B4 `bcaceeb`, B8 `a9baa51`, follow-through `ceece38`. QA's N46–N62 and N57 are in. QA is re-filming R1–R5 against it.

**The finding worth keeping: two correct rulings made a third thing impossible.** B1 (reverse today's night at check-out) and B3 (refuse a check-out in credit) were ruled on separately and are each right. Together they killed transfer-to-receivable: the desk settles *before* check-out, so the transfer sent the whole balance, the company was invoiced for a night nobody slept, and the guest's folio went into credit — where B3 refused the check-out outright. R3's check-out beat, dead, and no single ruling was wrong. Fixed with one server-side answer to "what must they settle to walk out today" that both the settle dialog and the transfer ask. The lesson is about the shape of the review, not the code: rulings were verified one at a time, and the interaction only showed up because QA filmed a journey that crossed both.

**Deliberate deviations from the review, both recorded in the commits:** B4 has no posted-night guard (the roll posts tonight before anybody moves anybody, so it would make an in-house move impossible — the one case the command exists for). B1's same-day case went round twice: architect's first message said zero nights, product's amendment `79631fd` says one, and **QA caught dev mid-flight with the wrong one**.

**The guard that was green over its own bug class.** `codes.test.ts` matches a RuleCode union up to the first semicolon, and a semicolon inside a member's doc comment ended the union early — so every member after it went unchecked, silently, in exactly the kind of code the guard exists for. It strips comments now. Nothing else was hiding.

**Open, with product (dev asked, not blocking the demo):** the literal reading of §10 6a for a multi-night stay leaving on arrival day; and whose language a *stored* memo is in (N51/N52 — the night line and the transfer memo are written into the ledger and are what a printed bill says next year; the raw company slug in N52 is a bug either way). **Wave 2 is unblocked** whenever architect opens it.
