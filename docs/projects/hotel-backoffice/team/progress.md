...
| WS1 stack → ES skeleton | solex-dev | **skeleton live** — events + projections + replay, Room aggregate, `/system` console; **auth live (D-11)**; building rough end-to-end in slices (dev profile rev 6): 0 foundation (Room flip + wipe + envelope + commandId) → 1 occupancy loop → 2 setup min → 3 money → 4 roles/audit → 5 long tail. Alex reviews staging after 1 and 3; architect reviews every slice (`team/qa.md`) |
