...
Rules: **check-in normalizes the stay to today**: tonight (current business date) must be a night of the stay — early arrival (before `arrive`) adds nights `[today, arrive)` at the first night's rate, room must be free for them, availability versioned, warn + allow; late arrival drops unposted nights before today (posted ones stay). Emitted as `stay.nights_changed` before `stay.checked_in`. So a checked-in stay holds ≥ 1 night while in house; same-day in/out reverses tonight at check-out (6a) · check-in needs tonight's room assigned and not out of order · check-out guards the stay's **own** folio only (0 or moved to a receivable); the master folio is guarded at `CloseBooking`, seen and settled on the booking page · **early check-out** (before `depart`) drops every night from the current business date on, in the same batch (`stay.nights_changed {removed}`, reversal of the current night's posted room charge, then `stay.checked_out`) so they go back on sale and the guest pays only nights slept (§10 6a, review B1) — never implicit in `checked_out` · nights change only before check-out · posted nights are immutable (no rate/room change) · no-show only from `booked`, after arrival date · cancel only if not checked in, reason required, charges moved off first (ezFolio guards).
...
| 6a | Early check-out | nights are `[arrive, depart)` (D-7): the departure date is not a night. Checking out on business date D (before the planned `depart`) removes every night `≥ D` — the current night included — in the same batch: `stay.nights_changed {removed}` frees them for sale and the current night's posted room charge is **reversed** (`folio.charge_voided {cause: earlyCheckOut}`) so the guest pays only the nights slept. Explicit events, never implied by `checked_out`. Leaving after `checkOutTime` on the last day is a **late check-out fee = catalogue item**, never an extra night (rule 1); staying past the next roll is the extra night. (Review B1, 2026-09-25.) |
| 6c | Early / late check-in | Check-in makes today the first unposted night: early arrival adds nights `[today, arrive)` (warn + allow, room must be free), late arrival drops unposted nights before today. A checked-in stay never has zero nights while in house; tonight is held and posted at check-in. A guest who checks in and out on the same business date has slept no night: 6a reverses tonight's charge and the stay ends with zero nights charged — day-use is out of scope, a late check-out item is the fee if one is wanted. |
...
| `CheckOut {stayId}` | `stay.check_out` | checkedIn · own folio 0 or transferred **after** the early-check-out reversal below | `stay.nights_changed {removed: nights ≥ today}` + `folio.charge_voided` (today's room line) if leaving before `depart` (versions availability), `stay.checked_out`, `folio.closed`, `room.marked_dirty` |
