...
- 2026-09-25: dev parked N51/N52 for architect's wave 2 (schema change, not a filming fix). Dev sizing: print/receipt locale threading ~1h (server-rendered, reads viewer locale today); company-name-at-render alone = couple of lines, demo-safe, fixes N52 slug. Base solex ceece38.
- 2026-09-25: R6–R9 'In ezFolio today' filled; QA's duplicate empty headings removed from R1–R5. Rulings: N64 erase does not cascade (page links the same-phone/name contact); receipt print = in scope, follow-up after demo. Architect unreachable — QA's 'all nine filmed on c4b0350, player player.html + player/0..8.js, republish artifact RrF94hDrGJ8QWPaecge4xN' needs relaying.
