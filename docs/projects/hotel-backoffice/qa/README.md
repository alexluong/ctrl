...
`N<n>` ids are minted here, by solex-qa only (architect ruling 2026-09-24); others describe a finding and ask for a number. Never reuse. **Next free: N16.** (N12 = guest search in URL, N13 = mise re-applies `.env` over exported env.)
