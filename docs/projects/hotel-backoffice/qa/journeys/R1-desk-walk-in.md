...
| # | Step (caption) | Notice |
|---|---|---|
| 1 | A walk-in wants a room tonight; which rooms are ready? | The room map's status filters, each with its count; "Ready" narrows the tiles. |
| 2 | Open 302 | The panel: state, guest (nobody), tonight's rate. No page change. |
| 3 | New booking: name, today, two nights, room 302 → book | The room type picks the rate from the rate table; the form shows the number of nights and the price source. |
| 4 | Stay page → Check in | Tonight's room charge is on the bill the moment the guest is in. |
| 5 | 2 Cokes from the minibar → Add to bill | A line with category, quantity and unit price; the totals strip updates. |
| 6 | Next morning the guest leaves a day early → Check out | The settle dialog asks for one night and the Cokes (540,000). The unused night goes back on sale (B1). |
| 7 | Paid and checked out | The stay now reads 1 night; the bill is closed and says why. |
| 8 | Back to the map | 302 is vacant and dirty, waiting for housekeeping. |
