...
Nine short screen recordings, one user-visible flow each (mvp.md §2, D-30). Filmed in English on solex d10cae5 (staging 8cbd00c0) from `solex/e2e/journeys/*.journey.ts` with `pnpm journeys`. Each one ships as a `.webm` and an rrweb replay; the player lists them all. Read these pages beside the player.
...
| R8 | [Cancellation path](R8-cancellation.md) | Oanh | 1:06 |
| R9 | [Back office](R9-back-office.md) | Oanh | 1:26 |
...
- **Film between 00:00 and 23:00 hotel time.** The day-start shift can't move the day after 23:00 (whole hours only); `dayStartsAt` refuses rather than film on the wrong day.
