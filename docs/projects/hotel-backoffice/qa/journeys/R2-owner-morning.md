...
|---|---|---|---|
| 1 | 0:02 | Dashboard | Today at a glance: occupancy, arrivals, what was sold, and the Needs attention list. |
| 2 | 0:05 | Revenue report | Revenue by category and by payment method. |
| 3 | 0:11 | Who owes money? → Blue Sea's statement | Outstanding per company, and the lines behind it. |
| 4 | 0:17 | Record the bank-transfer payment, with its reference | The company's balance goes to 0. |
| 5 | 0:23 | Room 103 via map → panel → stay | Three clicks from the map to a guest's bill. |
| 6 | 0:29 | Void the wrong laundry line, with a reason | The reason is required. The line stays, struck through with the reason, and leaves the total. |
| 7 | 0:38 | The stay's history | Booked, checked in, every bill line: who did it and when. |
| 8 | 0:42 | Room 103's own page | Who is in it tonight, the next arrival, housekeeping, and the room's own change log. |
