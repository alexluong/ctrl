...
- 2026-08-22 — **Dropbox re-checked (live API, VM creds).** Still full, in fact **over quota: 2,374,984,455 B used vs 2,147,483,648 B allocated (110%)**. Account holds exactly **9 files, all ibabot IBA exports** (`/exports/1346036658616930338-export-YYYYMMDD-HHMMSS.json`, ~250 MB each), server_modified 2026-08-20 06:51 → 2026-08-21 15:46 UTC. **eldobot: zero files, zero bytes** — R2 migration confirmed clean, Dropbox is now 100% ibabot's. Corrects an earlier assumption: ibabot is *not* accumulating unboundedly (only a 36h window of files survives, so something prunes), but retention is still far too loose for 2 GiB — ~4-5 uploads/day × 250 MB fills it immediately. ibabot is **actively in use**, not abandoned. Its own uploads are presumably failing intermittently while over quota. Secret still not rotated; rotating remains zero-cost to eldobot.
